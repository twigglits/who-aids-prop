...
double EventCondom::getNewInternalTimeDifference(GslRandomNumberGenerator *pRndGen, const State *pState)
{
	...
	if (evtTime < 0 || dt < 0)
		abortWithMessage("EventCondom::getNextInterventionTime: the next intervention takes place at time " + doubleToString(evtTime) + " which is before the current time " + doubleToString(population.getTime()) + " (dt = " + doubleToString(dt) + ")"); 
...
list<double> EventCondom::m_interventionTimes;
list<ConfigSettings> EventCondom::m_interventionSettings;
bool EventCondom::m_interventionsProcessed = false;

bool EventCondom::hasNextIntervention()
{
...
double EventCondom::getNextInterventionTime()
{
	assert(m_interventionTimes.size() == m_interventionSettings.size());
...
void EventCondom::popNextInterventionInfo(double &t, ConfigSettings &config)
{
	assert(m_interventionTimes.size() == m_interventionSettings.size());
	assert(m_interventionTimes.size() > 0);
...
ConfigFunctions EventCondomConfigFunctions(EventCondom::processConfig, EventCondom::obtainConfig,
		                                    "EventCondom", "initonce");

JSONConfig EventCondomJSONConfig(R"JSON(
        "EventCondom": { 
            "depends": null,
        ...
        "EventIntervention_enabled": { 
            "depends": [ "EventCondom", "intervention.enabled", "yes"],
            "params": [ 
