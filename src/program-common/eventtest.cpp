...
EventTest::EventTest(Person *pPerson1) : SimpactEvent(pPerson1)
{
...
string EventTest::getDescription(double tNow) const
{
...
list<double> EventTest::m_interventionTimes;
list<ConfigSettings> EventTest::m_interventionSettings;
bool EventTest::m_interventionsProcessed = false;

bool EventTest::hasNextIntervention()
{
...
double EventTest::getNextInterventionTime()
{
	assert(m_interventionTimes.size() == m_interventionSettings.size());
	assert(m_interventionTimes.size() > 0);
...
void EventTest::popNextInterventionInfo(double &t, ConfigSettings &config)
{
	assert(m_interventionTimes.size() == m_interventionSettings.size());
	assert(m_interventionTimes.size() > 0);
...
ConfigFunctions interventionConfigFunctions(EventTest::processConfig, EventTest::obtainConfig,
		                                    "EventTest", "initonce");
...
JSONConfig interventionJSONConfig(R"JSON(
        ...
        "EventTest_enabled": { 
            "depends": [ "EventTest", "intervention.enabled", "yes"],
            "params": [ 
